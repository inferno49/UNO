...
<i>The best Uno game around!!</i>
  ...
  ArrayList<Card> CardsInHand2 = new ArrayList<Card>();
  ...
  {
	  session.setAttribute("UnoCalled"," ");
	  if((request.getParameter("status") ==null))
	  {
       CardsInHand2 = newGame.SeeCards();
	  }
	  if((request.getParameter("status") !=null)&&(request.getParameter("status").equals("2")))
	  {
	   if(CardsInHand2.size()==1)
	   {
	  ...
	  out.print("</table>"); 
	 //session.setAttribute("gettop",null);
	  if(session.getAttribute("gettop")==null)
	  {
		  Card topcard;
		  topcard = newGame.gettop();
		 session.setAttribute("gettop",topcard.printCard()); 
		 
       } 
	  
	  out.print("<form action=startgame1.jsp?flag=0&status=3 method=post> <input name =button type=submit value=Draw a Card></input> </form> <br></br> <form action=startgame1.jsp?flag=1 method=post> <input name =button type=submit value=Play a Card></input> </form>");
  ...
  { 
	  int i = 0;
	  CardsInHand2 = newGame.AddCard(CardsInHand2);
	  ...
	  out.print("</table>"); 
	  response.addHeader("Refresh","10;https://localhost:8443/Uno_Game/JSP/Welcome/startgame1.jsp?flag=3&status=1");
	  
  }
  else if (request.getParameter("flag").equals("1"))  // get index and play a card
  {
	  int i = 0;
	  //CardsInHand2 = AddCard(CardsInHand2,newGame);
	  out.println("<h1>Your Cards</h1>");
	  out.print("<table border=1>");
	  ...
	  out.println("<form name ='indexform' action=startgame1.jsp?flag=2&status=3 onsubmit='return validateForm()' method= post > <label> Enter Index of the card you want to play</label> <input name=index type=text id=index maxlength=25 > </input> <input type=submit value=Play></input></form>");
  ...
  else if (request.getParameter("flag").equals("2"))  // Play a card 
  {
	  ...
	  if(j<CardsInHand2.size())
	  {
		Card select = CardsInHand2.remove(j);
		 ...
		 if(!top.contains(select.getColor().toString()) && (!top.contains(Integer.toString(select.getNumber()))))
		 {
			 ...
			 CardsInHand2.add(j,select);
			 response.addHeader("Refresh","4;https://localhost:8443/Uno_Game/JSP/Welcome/startgame1.jsp?flag=3&status=1");
		 }
		 else
		 {
